...
class MapiClient {
	...
	private function makeRequest($path, $method = "GET", $opts = array(), $params = array()){	
		 ...
		 * oAuth
		 */
		$consumer = new MPOAuthConsumer($this->_consumerKey, $this->_consumerSecret);
		$token = new MPOAuthToken($this->_token, $this->_tokenSecret);
		
		$request = MPOAuthRequest::from_consumer_and_token($consumer, $token, $method, $url);
		
		foreach ($params as $name => $value) {
			$request->set_parameter($name, $value);
		}		
		
		$request->sign_request(new MPOAuthSignatureMethod_HMAC_SHA1(), $consumer, $token);
	 	...
	 	if ($result === false){
			...
			throw $e;
	 		...
	 		if(isset($arrResult['response']['erro']) && $arrResult['response']['erro'] == 1){
	 			$e = new MapiException(
	 				array(
						'error_code' => $arrResult['response']['status'],
						'error'      => array(
